...
def analyze(
    ...
    try:
        ...
        if "all" in formats or "stix" in formats:
            ...
            stix_data = generate_stix_bundle(
                result.session_id,
                result.sample_sha256,
                result.sample_path,
                result.findings,
            ...
            stix_path.write_text(json.dumps(stix_data, indent=2))
...
def export(
    ...
    try:
        if output_format == "navigator":
            ...
            # Build API call records with proper JSON handling
            api_calls = []
